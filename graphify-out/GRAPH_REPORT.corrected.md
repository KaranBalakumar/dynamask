# Graph Report - dynamask_vio/models (corrected)  (2026-04-15)

## Corpus Check
- Corpus is ~7,911 words - fits in a single context window. You may not need a graph.

## Summary
- 147 nodes · 196 edges · 10 communities detected
...
3. `FlowDecoder` - 8 edges
4. `DifferentiablePreintegrator` - 8 edges
5. `DynaMaskVIO` - 8 edges
6. `BasicEncoder` - 8 edges
7. `FeatureMLP` - 7 edges
8. `differentiable_ba()` - 6 edges
9. `CNNEncoder` - 5 edges
10. `ScoreHead` - 5 edges
...
- `IMUEncoder` --uses--> `AirIMUCorrector`  [INFERRED]
  dynamask_vio/models/imu_encoder.py → dynamask_vio/models/airimu_corrector.py
- `IMU Encoder: AirIMU Corrector → Differentiable Preintegrator → Feature MLP.  Dat` --uses--> `AirIMUCorrector`  [INFERRED]
  dynamask_vio/models/imu_encoder.py → dynamask_vio/models/airimu_corrector.py
...
- **Why does `FlowDecoder` connect `Community 2` to `Community 8`, `Community 1`, `Community 5`?**
...
- **Are the 4 inferred relationships involving `IMUEncoder` (e.g. with `AirIMUCorrector` and `DifferentiablePreintegrator`) actually correct?**
  _`IMUEncoder` has 4 INFERRED edges - model-reasoned connections that need verification._
- **Are the 2 inferred relationships involving `FlowDecoder` (e.g. with `DynaMask V2.5 model wiring.` and `V2.5 optimizer groups with RAFT layerwise LR decay.`) actually correct?**
  _`FlowDecoder` has 2 INFERRED edges - model-reasoned connections that need verification._
